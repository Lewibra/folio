...
namespace PigWorldNamespace {
    ...
    public class Air {
        ...
        public void TransmitSound(int soundLevel) {
            ...
            //Loop around the 8 surrounding cells
            for (int i = 0; i < Direction.NUMBER_POSSIBLE; i++) {
                ...
                //if there's a wall, do nothing, else make sound
                if (adjacentCell == null) {
                } else {
                    //The sound will lose 1 int as it is being transmitted
                    int newSoundLevel = soundLevel - 1;
                    //Transmit the sound to the cells
                    Air air = new Air(adjacentCell);
                    air.Cell.Air.TransmitSound(newSoundLevel);
                }
