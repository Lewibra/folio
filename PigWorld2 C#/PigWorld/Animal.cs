...
namespace PigWorldNamespace {
    ...
    public abstract class Animal : LifeForm {
        ...
        /// in all immediately-surrounding cells and picking the one that has the gr(est
        /// sound level.
        /// 
        ...
        public Direction Listen() {
            ...
            //create lists to store all 8 directions and the sound levels they have
            List<int> soundLevel = new List<int>();
            List<Direction> directionList = new List<Direction>();
            //loops 8 times to find the direction and sound levels of each
            //adjacent cell
            for (int i = 0; i < Direction.NUMBER_POSSIBLE; i++) {
                ...
                //else, add the direction and sound level of the adjacent
                //cell to their lists
                if (adjacentCell == null) {
                } else {
                    soundLevel.Add(adjacentCell.Air.SoundLevel);
                    directionList.Add(direction);
                }
            }
            //if all values of sound found in the 8 cells are 0,
            //return null and do nothing
            //else, find the index of the highest sound level in it's list
            //and use it to find it's corresponding direction
            if ((soundLevel.Max() == 0) && (soundLevel.Min() == 0)) {
                return null; // Students should delete this "dummy" return.
            } else {
                //the index of the maximum sound is the index of the direction that is closest
                //to the female pig.
                int indexOfHighestSoundLevel = soundLevel.IndexOf(soundLevel.Max());
                Direction wayToGo = directionList[indexOfHighestSoundLevel];
